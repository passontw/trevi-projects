...
// ProvideRedisConfig 從 AppConfig 轉換為 go_gamecommon/cache 的 RedisConfig
...
// 提供 Redis 客戶端
func ProvideRedisClient(redisConfig *cache.RedisConfig, logger *zap.Logger) error {
	// 使用 redisConfig 初始化 Redis 連線
	logger.Info("提供 Redis 客戶端")
	err := cache.RedisInit(*redisConfig, 3*time.Second)
	if err != nil {
		logger.Error("Redis 客戶端初始化失敗", zap.Error(err))
		return err
	}
